...
module.exports = {
  regexModuleExistOnLine: /(?:^|\s)require\(['"]|^import\s.+from\s+["']|^import\s+["']|export\s+(?:\*|{[a-zA-Z0-9_$,\s]+})+\s+from|}\s*from\s*['"]/,
  ...
  regexOnCjsExport: function(prefix) {
    return new RegExp(`(?:var|const|let)\\s+\\{((?:.+?)*${escapeRegExp(prefix)})`, 'i');
  }
}
